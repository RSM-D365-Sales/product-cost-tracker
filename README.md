...
This is the **bluestem edition**: Bluestem Fresh Produce is RSM's fictional
grower-packer-shipper for the IFPA Global Produce & Floral Show. The item
master, sites, growers, suppliers and brand come from the Bluestem company pack
in `../Blustem-company-details` (`BRAND_GUIDE.md`,
`Bluestem_Brand_v2_and_Data.zip`); every figure is generated by the seed. The
title ribbon follows the brand guide — Midnight bar, bluestem logo on the left,
"Powered by" RSM sponsor mark at the right end before the avatar — and headings
are Poppins SemiBold (bundled, no CDN) over Segoe UI.
...
Item field — pick one from the lookup (`RAW-BLU` is the one the demo script
...
`?item=PK-BLU-PINT` — which is how they hand an item to each other.

### The demo data

Bluestem buys bulk fruit from its growers and a produce marketer and packs it
into retail and fresh-cut finished goods. All four items are set up for **batch
actual costing**, so a produced batch carries the actual cost of the
raw-material batch it consumed rather than a standard.

| Item             | Description                  | Unit | Source                                                  |
| ---------------- | ---------------------------- | ---- | ------------------------------------------------------- |
| `RAW-BLU`        | Blueberries, bulk            | lb   | Purchased — winter/spring import from Baja, cold chain  |
| `RAW-APL-HC`     | Apples, Honeycrisp, bulk     | lb   | Purchased — out of CA storage                            |
| `PK-BLU-PINT`    | Blueberries pint clamshell   | cs   | Produced — 9 lb of `RAW-BLU` per case of 12 pints        |
| `FC-APL-SLC-2OZ` | Apple Slices 2 oz snack cup  | cs   | Produced — 4 lb of `RAW-APL-HC` per case of 24 cups      |

Blueberries are received into the **Hart packing house (site `HRT`, warehouse
`HRT-RM`)** and packed there on the Berry & Cherry Pack Line; Honeycrisp are
received into the **Grand Rapids fresh-cut plant (`GRP` / `GRP-RM`)** and cut on
Apple Slicing Lines 1 and 2. About a fifth of finished volume is packed by the
contract packer in the **Holland DC (`HOL` / `HOL-CP`)**.

**V3014 Ridgeview Produce Exchange** is the lead marketer and carries every
anchor receipt (the one vendor not in Bluestem's pack — it has no produce
broker). Off-lead blueberry loads come straight from **G2028 Baja Fresca S.A.**
and **G2030 Central Coast Berry Co.**; `RAW-APL-HC` is deliberately
**dual-sourced** — Ridgeview books the cheap fruit on an 18-day CA-room
schedule, while **G2014 Van Dyke Vegetable Co.** trucks it off the Fruit Ridge
in 5 days at roughly a 7% premium (the `sourcing` mix on the catalogue entry,
with quoted lead times on the vendor records). The premium shows up in the
vendor column, the variance flags and Copilot's vendor-mix section: same
apples, different pipeline, and the price difference is the price of speed.

Ten hand-authored **anchor rows** carry the demo script and never move:

| Order       | Receipt      | Item          | Qty        | FOB / material | Charges | Landed  |
| ----------- | ------------ | ------------- | ---------- | -------------- | ------- | ------- |
| `PO-000241` | `PR-104812`  | `RAW-BLU`     | 42,000 lb  | $2.35          | $0.42   | $2.77   |
| `PO-000258` | `PR-105196`  | `RAW-BLU`     | 38,000 lb  | $2.35          | $0.31   | $2.66   |
| `PO-000273` | `PR-105644`  | `RAW-BLU`     | 40,000 lb  | $2.35          | $0.55   | $2.90   |
| `P000318`   | `PJ-000318`  | `PK-BLU-PINT` | 3,400 cs   | $24.93         | $3.63   | $28.56  |
| `P000341`   | `PJ-000341`  | `PK-BLU-PINT` | 3,100 cs   | $23.94         | $3.54   | $27.48  |
| `P000369`   | `PJ-000369`  | `PK-BLU-PINT` | 3,300 cs   | $26.10         | $3.76   | $29.86  |

Three blueberry receipts at an identical $2.35 FOB price land at $2.77, $2.66
and $2.90 — and the three pint runs that consumed them come out at $28.56,
$27.48 and $29.86 a case. Expand a production row to see which batch it
consumed. `PO-000273` is the load held at the Otay Mesa border crossing:
demurrage alone adds $0.06/lb, and it is the reason the 5/26 production run
(packed at the Holland co-packer) is the least profitable of the three.
...
A further 22 items (`FILLER_ITEMS` in [seed.ts](web/src/data/seed.ts)) sit behind
the four above — Gala apples, romaine, cucumbers, peppers, asparagus and squash,
ingredients, packaging and six more produced SKUs, all from Bluestem's item
master — bought from Bluestem's own growers and packaging suppliers (each item
maps onto the suppliers that would really carry it) and received at all four
sites, Salinas included. They carry no hand-authored rows and nothing in the
...
and a prospect who types a different item number gets a populated grid rather
than an empty one. 448 posted rows across 26 items in total.
...
`RAW-BLU` lands at $2.77, $2.66 and $2.90 across three receipts that all bought
blueberries at $2.35. That spread is the whole point of the inquiry, and it comes
...
Which charges an order attracts depends on what is on it: imported blueberries
pick up customs brokerage, border fees and pre-cooling; storage apples pick up
CA storage and handling instead; a mixed load gets only what applies to both.
...
`RAW-BLU` the purchase price climbs about $0.17 per lb per year while landed
cost climbs $0.20, because freight, fuel and border charges are inflating
faster than the blueberries are.
...
That last case is not hypothetical. Query `RAW-CUC` (Cucumbers, bulk) and the
fit collapses to about 3%, because a hard seasonal cycle is not a trend and a linear
...
Colours are the bluestem chart order — RSM Blue for landed cost, RSM Green for
purchase / material. Series identity never rests on colour alone: the checkbox row doubles as the legend and every value is
also in the grid.
...
purchase price, transportation, cold chain, customs & border, storage &
handling, inspection & testing. Expand a flagged row for the full decomposition; the cause deltas sum
...
what we've actually been paying". On `RAW-APL-HC`, `PO-000946` is the one to
point at: a Van Dyke spot load landing in 4 days at a FOB ~7% over the Ridgeview
loads either side of it.
...
On `RAW-BLU`, moving `PO-000931` out 7 days leaves a planned PK-BLU-PINT run short
...
`PK-BLU-PINT` is the one to pick; press **Run**.
...
`FC-APL-SLC-2OZ` Apple Slices 2 oz snack cup is the case worth demoing: the
item master carries $6.55, the roll-up at today's landed costs comes to
**$7.08** (+8.1%), and the margin a planner thinks they are making (40.2%) is
not the margin they are making (35.3%). The Honeycrisp on hand landed dearer
than the fruit the cost record was set from, and twenty-four cups and lids to a
case add up.
...
- *The bridge* sets the calculated cost against the quantity-weighted average of
  ...
  component and operation lines behind it: "packaging is up" becomes "cups are
...
Run `PK-BLU-PINT` on the two default Hart berry lines. The oldest lot expires
tomorrow and there are not enough committed hours to convert it: the plan puts
roughly **$41,700 of blueberries** (14,800 lb) in the at-risk block.

Open the parameters, tick **HOL-CP1 Co-pack line — Holland DC**, and run again.
At risk goes to zero, planned output rises by 1,600 cases, and a message bar points out
...
web/                          Vite + React 18 + TS + Tailwind
  ...
  src/lib/embed.ts            ?embed=1 / VITE_EMBED — drop the ribbon for F&SC embedding
...
*Product cost inquiry* — the three `RAW-BLU` anchors, that blueberries only ever
land in `HRT` / `HRT-RM`, the three `PK-BLU-PINT` production anchors, that a
...
*Branding and embedding* — that the bluestem ribbon shows by default with the
bluestem logo and the "Powered by" RSM sponsor mark, that its navigation pane
routes between the two inquiries, and that `?embed=1` drops it.
...
The bluestem ribbon shows by default, because the brand guide requires the RSM
sponsor mark on every app. When the workspace is hosted inside a real F&SC
environment (iframe or website host control on a workspace), the host already
has a bar and two of them reads as a mockup, so drop the ribbon with the embed
parameter ([`web/src/lib/embed.ts`](web/src/lib/embed.ts)). Cross-page
navigation stays available through the action pane buttons.

```
.../index.html?embed=1#/product-cost      per URL, no rebuild
.../index.html#/product-cost?embed=1      also accepted; latched for the session
VITE_EMBED=1                              in web/.env, to fix a build that way
```
